...
export default function TenantFailedNotificationsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetchWithTimeout('/api/notifications/failed')
      const json = (await res.json()) as { items?: FailedRow[]; error?: string }
  ...
  return (
    <AppShell isMobile={isMobile}>
      ...
      <div style={styles.content}>
        {!isMobile && (
          <PageHeader title="כשלי SMS / WhatsApp" subtitle="הודעות שלא נשלחו אחרי כל הניסיונות" />
        )}
        ...
        {loading ? (
          ...
          <Card>
            <p style={styles.empty}>אין כשלונות — מצוין.</p>
          </Card>
        ) : (
            ...
            {items.map((row) => (
              <Card key={row.id}>
                <div style={styles.rowTop}>
                  ...
                  <time style={styles.date} dateTime={row.created_at}>
                    {new Date(row.created_at).toLocaleString('he-IL')}
                  </time>
                </div>
                {row.destination && <p style={styles.dest}>יעד: {row.destination}</p>}
