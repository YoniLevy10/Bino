...
3. **SMS פיילוט לדיירים** — `pilot_sms` (במסך פרויקט)  
4. **תיקיית מסמכים** — `project_documents` (במסך פרויקט)  
...
## פיילוט SMS לדיירים (פרויקט) — תוסף בתשלום

- **UI:** פרויקטים → פתיחת פרויקט → «פיילוט — הודעת פתיחה לדיירים» (אם `pilot_sms` פעיל)
- **API:** `POST /api/projects/pilot-sms` — `dry_run: true` לספירת נמענים
- הודעה רב-לשונית, **ללא אימוג'י** (019SMS)
- לוג: `project_pilot_sms_runs`

## תיקיית מסמכים לפרויקט — תוסף בתשלום

- **UI:** אותו מגירת פרויקט → «תיקיית מסמכים» (אם `project_documents` פעיל)
- **API:** `GET/POST/DELETE /api/projects/documents`
- Storage: bucket `project-documents` (עד 15MB)
- מיגרציה: `054_project_documents_and_pilot_sms.sql`
