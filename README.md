...
**ניווט:** סדר פריטי התפריט נשמר ב-`clients.sidebar_nav_order` (מיגרציה `048`). במובייל — 4 פריטים קבועים בתחתית (בית, תקלות, פרויקטים, עובדים) + תפריט «עוד» לשאר.

**אבחון פלטפורמה (לא ללקוחות):** `/error-logs` ו-`/failed-notifications` קיימים בקוד; `middleware` מפנה אותם ל-`/` — התראות תפעול ל-`PLATFORM_OPS_EMAIL` (מיגרציה `049`). צוות פנימי: `/superadmin` (סוד מנהל).
