...
const CATALOG: Record<PaidAddonId, PaidAddonCatalogEntry> = {
  [PAID_ADDON_KEYS.calendar]: {
    ...
    marketing: {
      ...
      intro:
        'משרדי אחזקה מנהלים עשרות פגישות בחודש: ועדי בית, דיירים, קבלנים, ביקורי שטח. יומן המשרד Bino מרכז את כל האירועים לצד הפרויקטים והתקלות.',
      valueProps: [
  ...
  [PAID_ADDON_KEYS.collections]: {
    ...
    description:
      'יצירת חיובים ודרישות תשלום ב-Grow — הכסף נכנס לחשבון Grow שלכם, לא דרך Bino.',
    ...
    marketing: {
      ...
      intro:
        'כל חברת ניהול פותחת חשבון Grow משלה. Bino יוצרת דרישת תשלום, שולחת SMS לדייר, ומסמנת «שולם».',
      valueProps: [
        'חשבון Grow משלכם — לא חשבון פלטפורמה',
        'הכסף נכנס ישירות אליכם',
        'קישור תשלום ב-SMS דרך Bino',
